...
{
    uECC_word_t x[ECC_BYTES/8];
    uECC_word_t y[ECC_BYTES/8];
} EccPoint;
...
struct uECC_Curve_t {
  int num_words;
  int num_digits;
  uECC_word_t p[ECC_BYTES/8];
  uECC_word_t n[ECC_BYTES/8];
  EccPoint G;
  uECC_word_t b[ECC_BYTES/8];
};
...
typedef enum curve_type_t curve_type;
...
*/
int ecc_make_key(uint8_t p_publicKey[ECC_BYTES+1], uint8_t p_privateKey[ECC_BYTES], curve_type curve_type);
